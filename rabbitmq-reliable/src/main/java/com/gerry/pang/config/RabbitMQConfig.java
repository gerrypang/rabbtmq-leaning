...
@Configuration
@EnableRabbit
@AutoConfigureAfter(MybatisMapperScanerConfig.class) // 在数据源之后在加载配置
public class RabbitMQConfig {
	...
	public AmqpTemplate rabbitTemplate(ConnectionFactory connectionFactory) {
		...
		/**
		 * 为true 时，交换器无法根据类型和路由键找到符合的队列，调用Basic.Return 命令将消息返回给生产者。 为false
		 * 时，出现上述情形，则消息直接被丢弃。
		 */
	...
	private void confirmCallback(CorrelationData correlationData, boolean isAck, String cause) {
		if (!isAck) {
			log.error("====== 发送消息失败，messageId:{}，失败原因：{} ====== ", correlationData.getId(), cause);
		} else {
			log.info("====== 发送消息成功 ======");
		}
	}
	...
	private void returnedMessage(Message message, int replyCode, String replyText, String exchange, String routingKey) {
		log.info("returnedMessage message:{}, replyCode:{}, replyText:{}, exchange:{}, routingKey:{}", message,
